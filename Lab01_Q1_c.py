...
Original file is located at
    https://colab.research.google.com/drive/1ZhpBZNZ6OlElWvnyw6BP2an3v_xkMXY7
"""

#Purpose: To visualize Murcury's orbut using the Euler-Cromer method of integration.
...
#For Mercury the inital conditions are
...
# DEFINE the values of various parameters: m, M_s, G in appropriate units.
...
def dot_r_advance(dot_r,r,dot_theta, dt):
    return dot_r + (r * dot_theta**2)*dt - (G*M_s/(r**2))*dt
...
dt = 0.0001
times = np.arange(start=beg, stop=end, step = dt)

#CREATE four empty arrays and append r_0 to one and \dot r_0 to one, \theta to one and \dot_\theta to the last.
#CREATE an empty array and append the angular momentum to it. - We will track if this value is actually conserved in the simulation as it should be.

position = [r_0]
angle = [theta_0]
radial_velocity = [dot_r_0]
angular_velocity = [dot_theta_0]
...
L_rms = np.sqrt(np.average(np.power(np.subtract(Angular_Momentum,Angular_Momentum),2))) # square root of the average of the squares of the deviations from the initial angular momentum

print("RMS Deviation of angular momentum: {0:.5e}".format(L_rms)) # check if constant

#Draw a PLOT of the orbit.
plt.plot(x,y,ls='-',label="path of Mercury")
